...
export function getAllPosts(postsOnly = false) {
  //reads all file names of the directory
  const postFiles = fs.readdirSync(postsDir);
  let allPosts = postFiles.map(file => {
    return getPostData(file);
  });
  if (postsOnly === true) {
    allPosts = allPosts.filter(post => post.isPost === true);
  }
...
export function getAllApps() {
  const allPosts = getAllPosts();
  const myApps = allPosts.filter(post => post.isApp === true);
  return myApps; 
}